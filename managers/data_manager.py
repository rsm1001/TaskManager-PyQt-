    ...
    def search_all_tasks(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        for task in self.get_daily_tasks():
            if self._task_matches(task, keyword_lower, ['title', 'description', 'tags', 'category']):
                task_dict = self.daily_task_manager.to_dict(task)
        ...
        for task in self.get_todo_tasks():
            if self._task_matches(task, keyword_lower, ['title', 'description', 'tags', 'category']):
                task_dict = self.todo_manager.to_dict(task)
        ...
        for task in self.get_entertainment_tasks():
            if self._task_matches(task, keyword_lower, ['title', 'description', 'tags', 'category']):
                task_dict = self.entertainment_manager.to_dict(task)

...
def test_auto_navigation_prefers_latest_overdue_pending_slot():
    app = _app()
    widget = ItineraryWidget(data_manager=None)
    widget._day_views[0].get_slot(22).add_task(_task("monday-pending"), persist=False)
    widget._day_views[1].get_slot(9).add_task(_task("latest-pending"), persist=False)
    ...
    target_block = next(block for block in widget._day_views[1].blocks if block.start_hour == 8)
    target_slot = target_block.get_slot(9)
